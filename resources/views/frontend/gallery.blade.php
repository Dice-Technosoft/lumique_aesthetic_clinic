  ...
  <div class="container-luxury">
    ...
    <div class="grid-3" id="galleryGrid">
      ...
      <div class="luxury-card reveal gallery-case-card" data-category="{{ strtolower($case->category ?? 'general') }}" style="overflow: hidden; border-radius: 6px; display: flex; flex-direction: column; justify-content: space-between; border: 1px solid var(--color-border); background: #ffffff;">
        ...
        <div style="padding: 0.9rem 1.25rem; border-top: 1px solid var(--color-border); background: var(--color-ivory); display: flex; justify-content: space-between; align-items: center; font-size: 0.8rem; gap: 0.75rem;">
          <span style="color: var(--color-charcoal-muted); font-size: 0.8rem; line-height: 1.3;">Category: <strong style="color: var(--color-charcoal);">{{ $categories->firstWhere('slug', $case->category)->name ?? ucfirst(str_replace('-', ' ', $case->category ?? 'General')) }}</strong></span>
          <button type="button" class="btn-primary btn-sm" onclick="openAppointmentModal('{{ addslashes($case->treatment_name ?: $case->title) }}')" style="white-space: nowrap; flex-shrink: 0; font-size: 0.72rem; padding: 0.45rem 0.85rem; border-radius: 4px; display: inline-flex; align-items: center; gap: 0.35rem; text-transform: uppercase; font-weight: 600; letter-spacing: 0.04em; background: var(--color-crimson); color: #ffffff !important; border: 1px solid var(--color-crimson); cursor: pointer; transition: all 0.25s ease; box-shadow: 0 2px 8px rgba(122, 28, 46, 0.25);" onmouseover="this.style.backgroundColor='var(--color-burgundy)'; this.style.borderColor='var(--color-burgundy)'; this.style.transform='translateY(-1px)';" onmouseout="this.style.backgroundColor='var(--color-crimson)'; this.style.borderColor='var(--color-crimson)'; this.style.transform='none';">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink: 0;"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>
